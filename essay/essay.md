...
可以看出一旦$\alpha$, $\beta$到正数区间内，拟合优度的变化就趋于平缓。而且r-value的变化对于$\alpha$的变化更加敏感。而且在$\alpha, \beta$处于合适的区间内时，拟合优度处于一个0.8以上的高位。因此我们可以得出，以代码得分为主要评价方式，代码行数，代码运行时间作为辅助评价手段，是对代码能力的一个合适的评价方式。
